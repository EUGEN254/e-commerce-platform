...
import {
  ...
  extractPublicId,
  backgroundCleanupProductImages,
  backgroundCleanupMultipleProductImages,
...
const createProduct = async (req, res) => {
  try {
    ...
    const missingFields = Object.entries(requiredFields) // Convert object to entries
      .filter(([field]) => !req.body[field] && req.body[field] !== 0) // Filter fields that are missing while allowing zero values
    ...
    // Parse JSON or CSV strings into arrays/objects
        ...
        try {
          ...
          if (data.includes(",")) {
            return data
              .split(",")
              .map((item) => item.trim())
              .filter(Boolean); //boolean removes empty strings
          }
          return [data.trim()];
        }
      }
    ...
    // Upload main image to Cloudinary and set secure URL
    let mainImageUrl = "";
    if (mainImageFile) {
      try {
      ...
      try {
        const uploadPromises = additionalImageFiles.map((file, index) =>
          cloudinary.uploader.upload(file.path, {
            ...
            public_id: `${name
              .replace(/\s+/g, "-")
              .toLowerCase()}-additional-${Date.now()}-${index}`,
            overwrite: false,
          })
        );

        const additionalResults = await Promise.all(uploadPromises);
        additionalImageUrls = additionalResults.map(
    ...
    const productData = {
      ...
      inStock:
        inStock === true || inStock === "true" || (parseInt(stock) || 0) > 0,
...
const getProductById = async (req, res) => {
  try {
    ...
    const product = await Product.findById(id).select("-__v -createdBy").lean();
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (mainImageFile) {
      try {
        ...
        if (existingProduct.mainImage) {
          ...
          if (publicId) {
            const fullPublicId = `ecommerce/products/main/${publicId}`;
            await safeDeleteCloudinaryResource(fullPublicId);
          }
        }
        ...
        const mainImageResult = await cloudinary.uploader.upload(
          ...
          {
            folder: "ecommerce/products/main",
            public_id: `${(name || existingProduct.name)
              .replace(/\s+/g, "-")
              .toLowerCase()}-main-${Date.now()}`,
            overwrite: false,
          }
    ...
    if (additionalImageFiles.length > 0) {
      try {
        // Delete old additional images from Cloudinary with timeout and retry handling
        if (existingProduct.images && existingProduct.images.length > 0) {
          for (const imageUrl of existingProduct.images) {
            if (imageUrl && imageUrl !== existingProduct.mainImage) {
    ...
    const updateData = {
      ...
      ...(features && {
        features: parseArray(features) || existingProduct.features,
      }),
...
const deleteProduct = async (req, res) => {
  try {
    ...
    // DELETE FROM DATABASE FIRST (fast response to admin)
    await Product.findByIdAndDelete(id);

    // CLEANUP CLOUDINARY IMAGES IN BACKGROUND (fire-and-forget)
    // This runs after the response is sent, so admin gets instant feedback
...
const bulkDeleteProducts = async (req, res) => {
  try {
    ...
    // Find products to get their images (before deleting)
    ...
    if (products.length > 0) {
      backgroundCleanupMultipleProductImages(products);
    }

    logger.info("Successfully deleted", result.deletedCount);
